...
void Simulator::deal_cards(std::array<Card, 2>& p0_cards, std::array<Card, 2>& p1_cards) {
...
std::vector<Card> Simulator::deal_board() {
    // Create and shuffle deck
    std::vector<Card> deck;
    deck.reserve(52);
    for (Card c = 0; c < 52; ++c) {
...
int Simulator::simulate_betting_round(
    ...
    // REAL POKER: Unlimited raises until someone calls or folds
    // Both players can bet/raise multiple times
    
    int current_player = 0;  // Start with player 0
    int last_raiser = -1;    // Track who last raised
    int raise_count = 0;     // Count raises this round
    int max_raises = 3;      // Typical poker cap (bet + 2-3 raises)
    int actions_this_round = 0; // Track total actions to prevent infinite loops
    ...
    while (raise_count < max_raises && actions_this_round < 20) {
        ...
        if (action == Action::FOLD) {
            // Settle bets into pot before returning
            ...
            int min_raise = std::max(1, to_call + 1);  // Must raise at least 1 more
            int desired_total = std::max(current_bets[current_player] + to_call + bet_size, 
                                        current_bets[current_player] + min_raise);
            
            // Calculate how much more to add (not total bet)
            int additional_amount = std::min(desired_total - current_bets[current_player], 
        ...
        if (current_bets[0] == current_bets[1] && last_raiser != current_player) {
            // Both players have equal bets and no one just raised
            break;
        }
    ...
    // Settle all bets into pot
    int total_bet = current_bets[0] + current_bets[1];
    pot += total_bet;
    current_bets[0] = 0;
    current_bets[1] = 0;
...
HandResult Simulator::simulate_hand(
    ...
    deal_cards(p0_cards, p1_cards);
    
    // Minimal betting simulation: 2-3 betting rounds
    std::vector<Card> board = deal_board();
    
    // Pre-flop betting (simplified)
    int preflop_action = simulate_betting_round(bot0, bot1, p0_cards, p1_cards, 
                                               std::vector<Card>(), pot, stacks, current_bets, result);
    if (preflop_action != -1) {
        // Someone folded
    ...
    // Post-flop betting (simplified) - only if both players still in
    int postflop_action = simulate_betting_round(bot0, bot1, p0_cards, p1_cards, 
                                                board, pot, stacks, current_bets, result);
